...
class OmniGen2Pipeline(DiffusionPipeline):
    ...
    def prepare_image(
        ...
        if len(images) > 3:
            raise ValueError(f"Number of input images must be <= 3, but got {len(images)}")

        latents = []
        for i, img in enumerate(images):
            if img is not None:
                if isinstance(img, list): # check if multiple images are passed for the same reference
                    ref_latents = []
                    ...
                    img = self.image_processor.preprocess(img, max_pixels=max_pixels, max_side_length=max_side_length)
                    ref_latents = self.encode_vae(img.to(device=device)).squeeze(0)
            else:
                ref_latents = None
    ...
    def __call__(
        ...
        if len(input_images) == 1 and align_res:
            if ref_latents[0] is not None:
                if isinstance(ref_latents[0], list):
                    width, height = ref_latents[0][0].shape[-1] * self.vae_scale_factor, ref_latents[0][0].shape[-2] * self.vae_scale_factor
                else:
                    width, height = ref_latents[0].shape[-1] * self.vae_scale_factor, ref_latents[0].shape[-2] * self.vae_scale_factor
